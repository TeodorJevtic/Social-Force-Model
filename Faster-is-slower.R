...
while(runcount<=maxruncount){
  ...
  xs=runif(N,2.9,4.9)                   #Combine to get the starting x-positions of all of them
  ...
  while(tcount<N){
    ...
    while(i<=N){
      ...
      while(i<=N){
        if(abs(ycurrent[i])<corridorw/2){
          Ftotx[i]=Fa[i,1]+Fpedwx[i]+Fpedwx2[i]+Fpedwx3[i]
          Ftoty[i]=Fa[i,2]+Fpedwy[i]+Fpedwy2[i]+Fpedwy3[i]
          i=i+1}
        else{
          Ftotx[i]=Fa[i,1]+Fpedwx[i]+Fpedwx2[i]+Fpedwx3[i]
          Ftoty[i]=Fa[i,2]+Fpedwy[i]+Fpedwy2[i]+Fpedwy3[i]
