...
while(runcount<=maxruncount){
  ...
  xs=runif(N,0,2.5)                   #Combine to get the starting x-positions of all of them
  ...
  while(tcount<N){
    ...
    while(i<=N){
      ...
      obstaclex=c(exitxpos,exitxpos)
      nobstacles=length(obstaclex)
      obstacley=c(corridorw/2,-corridorw/2)
      ...
      lines(x = radiusob * cos(theta) + exitxpos, y = radiusob  * sin(theta) + corridorw/2,asp=1,xlim = c(1,10),ylim=c(-10,40),col=2)
      par(new=TRUE)
      lines(x = radiusob * cos(theta) + exitxpos, y = radiusob  * sin(theta) - corridorw/2,asp=1,xlim = c(1,10),ylim=c(-10,40),col=2)
      par(new=TRUE)
      lines(x=corridorarg,y=numeric(200)+corridorw/2)
